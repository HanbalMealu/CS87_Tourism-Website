...
class AdminListUserPage extends Component {
    constructor(props){
        super(props);
        this.state = {
            userPage: {},
    ...
    componentWillMount(){
        ...
        this.sendid();
          setTimeout(() => {
            this.getusertours() ,this.getuserservices(),  this.getuserreviews()
          }, 1000);
         
      }
      sendid(){
         const op_id={
            od:this.props.location.state.userPage.id
        }
    ...
    render() {
      const msgloggedin=(
        <Link to= {{
          ...
          }}><button className="myPostCRUD">Message</button></Link>
      )
